...
public class SpriteToGameObjectEditor : EditorWindow
{
    ...
    private readonly string[] _faceOptions = {"Sprite", "Two Sided", "One Sided"};
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.Space(_customWidth);
        GUILayout.Label(
            "Render face doesnt apply to default sprite shader,",
            EditorStyles.label);
        GUILayout.Label(
            " if you want to use one sided select a different shader,",
            EditorStyles.label);
        GUILayout.Label(" such as \"Universal Render Pipeline/Lit\"",
            EditorStyles.label);
        EditorGUILayout.Space(_customWidth);
